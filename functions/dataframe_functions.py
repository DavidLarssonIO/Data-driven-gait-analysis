...
def ImportData(filepath,skier):
    ...
    df.columns = ["Time (sec)", "Force left (N)", "Force right (N)", "Time (floor min)", "Slope (%)", "Speed (km/h)", "Gear"]    
    df.loc[:,"Time (sec)"] = df.loc[:,"Time (sec)"]*60
    df["Skier"] = [skier]*len(df.index)
    return df

def GetStrokes(df_force,df_gear,df_time,df_skier,height=25,width=1,distance=50,rel_height=0.93,cutoff_start = 2, cutoff_end = 0):
    # height=25
    ...
    # Triming for cutoff
    index_trim = np.logical_and(ground_start >= cutoff_start,ground_start<=cutoff_end)
    ground_start = ground_start[index_trim]
    
    # Wanting to get a ground start as first value and a ground_stop as last value.
    index_trim = np.logical_and(ground_start[0] < ground_stop,ground_stop < ground_start[-1])
    ...
    for i in range(len(peaks)):
        ...
        impulse[i] = np.trapz(df_force[start:stop].values,df_time[start:stop].values)    
    data = {"Gear": gear.values,
            ...
            "Skier": skier,
            "Impulse": impulse}
    # TODO: Add orginal index dataframe
...
def GetInfo(df_peaks):
    stroke_time = df_peaks["Stroke stop"].values-df_peaks["Stroke and ground contact start"].values
    ground_contact_time = df_peaks["Ground contact stop"].values-df_peaks["Stroke and ground contact start"].values
    air_time = df_peaks["Stroke stop"].values-df_peaks["Ground contact stop"].values

    data = {"Gear": df_peaks["Gear"].values,
           "Stroke time": stroke_time,
           "Ground contact time": ground_contact_time,
           "Air time": air_time,
           "Frequency": 1/stroke_time,
           "Impulse": df_peaks["Impulse"].values,
...
def generate_stroke_dataframe(filepath,i):
    ...
    df_peaks_left = GetStrokes(df["Force left (N)"],df["Gear"],df["Time (sec)"],df["Skier"])
    df_peaks_right = GetStrokes(df["Force right (N)"],df["Gear"],df["Time (sec)"],df["Skier"])
    df_info_left = GetInfo(df_peaks_left)
    df_info_right = GetInfo(df_peaks_right)
    df_info_left, df_info_right = add_left_and_right_diff(df_peaks_left,df_peaks_right,df_info_left,df_info_right)
...
def add_left_and_right_diff(df_peaks_left,df_peaks_right,df_info_left,df_info_right):    
    nbrs_left = NearestNeighbors(n_neighbors=1,n_jobs=-1).fit(np.transpose([df_peaks_left["Peak time"].values]))
    nbrs_right = NearestNeighbors(n_neighbors=1,n_jobs=-1).fit(np.transpose([df_peaks_right["Peak time"].values]))
    times_right, _ = nbrs_left.kneighbors(np.transpose([df_peaks_right["Peak time"].values]))
    times_left, _ = nbrs_right.kneighbors(np.transpose([df_peaks_left["Peak time"].values]))
